...
function multiMap(arrVals, arrCallbacks) {
  ...
  for(let i=0;i<arrVals.length;i++){
    var sub = arrCallbacks.map((elem)=>elem(arrVals[i]));
    obj[arrVals[i]]=sub;
  }
  return obj;
}
...
function objOfMatchesWithArray(array1, array2, callback) {
  let obj={};
  for(let i=0;i<array1.length;i++){
    let sub = callback.reduce((acc,elem)=>{return elem(acc)},callback[0](array1[i]));
    if(sub===array2[i]){
      obj[array1[i]]=array2[i];
    }
  }
...
function objectWithArrayValues(array1, callback) {
  let obj={};
  for(let i=0;i<array1.length;i++){
    let sub = callback.map((elem)=>elem(array1[i]));
    obj[array1[i]]=sub;
  }
  return obj;
}
